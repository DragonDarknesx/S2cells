// ==UserScript==
// @id             iitc-plugin-l14cells@reboot
// @name           IITC plugin: Show Level 14 Cells
// @author         ReBootYourMind
// @category       Layer
// @version        0.1.5
// @namespace      https://github.com/vibrunazo/l14cells
// @updateURL      ..
// @downloadURL    ..
...
// Copied a plugin by vib with minor changes. Original here:
...
function wrapper(plugin_info) {
  ...
  plugin_info.buildName = 'l14cells';
  plugin_info.dateTimeVersion = '20170109.010105';
  plugin_info.pluginId = 'l14cells';
  ...
  window.plugin.showcells2 = function() {};

  var input=document.createElement("input");
  input.type="button";
  input.value="Set Level 2";
  input.onclick = setCellLevel;
  input.setAttribute("style", "font-size:18px;position:absolute;top:160px;left:40px;color:black;cursor:pointer;pointer-events:all;z-index:2999;");
  document.body.appendChild(input);

  window.plugin.showcells2.cellLevel = 14;

  function setCellLevel()
  {
   var newCellLevel = prompt("Set a cell level", "14");
   ...
   //alert("new cell=" + newCellLevel);
    if (newCellLevel !== isNaN && newCellLevel >= 2 && newCellLevel <= 20) {
      ...
      window.plugin.showcells2.cellLevel = newCellLevel;
      window.plugin.regions2.update();
    } else {
  ...
  // use own namespace for plugin
  window.plugin.regions2 = function() {};

  window.plugin.regions2.setup  = function() {
    ...
    window.plugin.regions2.regionLayer = L.layerGroup();
    ...
    addLayerGroup('Score Regions', window.plugin.regions2.regionLayer, true);

    map.on('moveend', window.plugin.regions2.update);

    addHook('search', window.plugin.regions2.search);

    window.plugin.regions2.update();
  };

  window.plugin.regions2.FACE_NAMES = [ 'AF', 'AS', 'NR', 'PA', 'AM', 'ST' ];
  window.plugin.regions2.CODE_WORDS = [
  ...
  window.plugin.regions2.REGEXP = new RegExp('^(?:(?:(' + plugin.regions2.FACE_NAMES.join('|') + ')-?)?((?:1[0-6])|(?:0?[1-9]))-?)?(' +
  plugin.regions2.CODE_WORDS.join('|') + ')(?:-?((?:1[0-5])|(?:0?\\d)))?$', 'i');

  window.plugin.regions2.regionName = function(cell) {
    ...
    var name = window.plugin.regions2.FACE_NAMES[cell.face];
    ...
    if (cell.level >= 4) {
      ...
      name += zeroPad(regionI+1,2)+'-'+window.plugin.regions2.CODE_WORDS[regionJ];
    }
  ...
  window.plugin.regions2.search = function(query) {
    var terms = query.term.replace(/\s+/g, '').split(/[,;]/);
    var matches = terms.map(function(string) {
      return string.match(window.plugin.regions2.REGEXP);
    });
    if(!matches.every(function(match) { return match !== null; })) return;

    var currentCell = window.plugin.regions2.regionName(S2.S2Cell.FromLatLng(map.getCenter(), 6));
    ...
    matches.forEach(function(match) {
      ...
      var result = window.plugin.regions2.getSearchResult(match);
      if(result) query.addResult(result);
    });
  };

  // rot and d2xy from Wikipedia
  window.plugin.regions2.rot = function(n, x, y, rx, ry) {
    if(ry == 0) {
      if(rx == 1) {
        x = n-1 - x;
        y = n-1 - y;
  ...
  window.plugin.regions2.d2xy = function(n, d) {
    var rx, ry, s, t = d, xy = [0, 0];
    for(s=1; s<n; s*=2) {
      rx = 1 & (t/2);
      ry = 1 & (t ^ rx);
      xy = window.plugin.regions2.rot(s, xy[0], xy[1], rx, ry);
      xy[0] += s * rx;
      xy[1] += s * ry;
      t /= 4;
    }
  ...
  window.plugin.regions2.getSearchResult = function(match) {
    var faceId = window.plugin.regions2.FACE_NAMES.indexOf(match[1]);
    var id1 = parseInt(match[2]);
    var codeWordId = window.plugin.regions2.CODE_WORDS.indexOf(match[3]);
    ...
    if(id2 === undefined) {
      ...
      var xy = window.plugin.regions2.d2xy(4, id2);
    ...
    result.title = window.plugin.regions2.regionName(cell);
  ...
  window.plugin.regions2.update = function() {

    window.plugin.regions2.regionLayer.clearLayers();
    ...
    var drawCellAndNeighbors = function(cell) {
      ...
      if (!seenCells[cellStr]) {
        ...
        if (cellBounds.intersects(bounds)) {
          // on screen - draw it
          window.plugin.regions2.drawCell(cell);
    ...
    var maxzoom = 16;
    if (window.plugin.showcells2.cellLevel <= 14) maxzoom = 10;
    if (window.plugin.showcells2.cellLevel <= 8) maxzoom = 5;
    if (zoom >= maxzoom) {  // 5 // ;;;;
      // var cellSize = zoom>=7 ? 6 : 4;  // ;;;;vib
      var cellSize = window.plugin.showcells2.cellLevel;
    ...
    for (var i=0; i<latLngs.length-1; i++) {
      ...
      window.plugin.regions2.regionLayer.addLayer(poly1);
      ...
      window.plugin.regions2.regionLayer.addLayer(poly2);
    }
    ...
    for (var i=-135; i<=135; i+=90) {
      ...
      window.plugin.regions2.regionLayer.addLayer(poly);
    }
  }

  window.plugin.regions2.drawCell = function(cell) {
    ...
    // name
    var name = window.plugin.regions2.regionName(cell);


    var color = cell.level == 6 ? 'red' : 'red';

    // the level 6 cells have noticible errors with non-geodesic lines - and the larger level 4 cells are worse
    // NOTE: we only draw two of the edges. as we draw all cells on screen, the other two edges will either be drawn
    // from the other cell, or be off screen so we don't care
    var region = L.geodesicPolyline([corners[0],corners[1],corners[2]], {fill: false, color: color, opacity: 0.7, weight: 7, clickable: false });

    window.plugin.regions2.regionLayer.addLayer(region);
    ...
    //window.plugin.regions2.regionLayer.addLayer(marker);  // ;;;;vib
  };

  var setup =  window.plugin.regions2.setup;
